...
function! _AutoCommandSetBaseText()
   let l:SymbolName={"%name": _GetNameFromBuf(bufname(),0),"%pname": _GetNameFromBuf(bufname(),1),"%type": _GetTypeFromName(bufname()),"%user": $USER,"%date1": strftime("%c"),                    "%date2":strftime('%Y-%m-%d'),"%date3":strftime('%A'),"%time":strftime("%H:%M")}
